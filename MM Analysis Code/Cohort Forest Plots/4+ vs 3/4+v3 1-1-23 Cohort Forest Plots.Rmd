...
```{r}
Infection <- read.csv("4+v3 Infection Non-Matched Forest Plot.csv") 
Hospitalization <- read.csv("4+v3 Severe Disease Non-Matched Forest Plot.csv") 
```
...
```{r}
pdf("./Figures/Cohort_4+v3_Infection_Start_Quarter.pdf", width = 14, height = 6, onefile = FALSE)
...
fn <- local({
  ...
  test_inclusion_ind <- ifelse(infection_plot$test_inclusion == "Y", yes = 1, no = 0)

  function(..., clr.line, clr.marker){
    i <<- i + 1
    if(test_inclusion_ind[i] == 1){
...
fig <- forestplot(infection_plot, 
           ...
           #title = "Cohort 4+ vs 3 Dose Infection VE with Different Non-Matched Designs",
           title = "",
...
legend(x = 0.21, y = 1.22, c("Adjusted", "Unadjusted"), title = "Adjustment", border = "#CCCCCC", box.lwd = 1.5, 
       col = c(pal[1], pal[2]), pch = c(15,15), bg = "white", horiz = TRUE, xpd = TRUE) 
legend(x = 0.51, y = 1.22, c("Included", "Not Included"), title = "Boosted In Prev. Quarters", border = "#CCCCCC", box.lwd = 1.5, 
       col = c("black", "black"), pch = c(15,16), bg = "white", horiz = TRUE, xpd = TRUE) 
...
```{r}
pdf("./Figures/Cohort_4+v3_Severe_Start_Quarter.pdf", width = 10, height = 6, onefile = FALSE)
...
fn <- local({
  ...
  test_inclusion_ind <- ifelse(hosp_plot$test_inclusion == "Y", yes = 1, no = 0)
...
fig <- forestplot(hosp_plot, 
                  fn.ci_norm = fn,
           labeltext = c(study_start, n, CI), 
           #title = "Cohort 4+ vs 3 Dose Severe Disease VE with Different Non-Matched Designs",
           title = "",
           xlab = "Vaccine Effectiveness (VE)",
           graph.pos = 3,
           boxsize = .15,
