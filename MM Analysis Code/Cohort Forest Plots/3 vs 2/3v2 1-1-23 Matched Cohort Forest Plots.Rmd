...
```{r}
Infection <- read.csv("3v2 Infection Matched Forest Plot.csv") 
Hospitalization <- read.csv("3v2 Severe Disease Matched Forest Plot.csv") 
...
#pal <- c("#377EB8", "#E41A1C", "#4DAF4A", "purple")
```

```{r}
pdf("./Figures/Cohort_3v2_Infection_Matched.pdf", width = 10, height = 8, onefile = FALSE)
...
```{r}
pdf("./Figures/Cohort_3v2_Severe_Matched.pdf", width = 10, height = 8, onefile = FALSE)
...
infection_plot <- bind_rows(header, infection_plot)
plot.new()
fig_infection <- forestplot(infection_plot, 
                  fn.ci_norm = fn,
           labeltext = c(method, n, CI), 
           #title = "Cohort 3 vs 2 Dose Infection VE with Different Matched Designs (Q4 2021 Index Date)",
           #title = "",
...
hospitalization_plot <- bind_rows(header, hospitalization_plot)
plot.new()
fig_severe <- forestplot(hospitalization_plot,
                  fn.ci_norm = fn,
           labeltext = c(method, n, CI), 
           #title = "Cohort 3 vs 2 Dose Severe Disease VE with Different Matched Designs (Q4 2021 Index Date)",
           #title = "",
...
p1 <- grid2grob(print(fig_infection))
p2 <- grid2grob(print(fig_severe))
p_both <- wrap_plots(p1, p2, nrow = 2)

pdf("./Figures/Cohort_3v2_Matched_Multipanel.pdf", width = 10, height = 14, onefile = FALSE)
plot.new()
p_both
legend(x = .275, y = 1.1, 
       c("Boosting", "Testing", "Both", "Neither"), title = "Matching/Propensity For", border = "#CCCCCC", box.lwd = 1.5, 
