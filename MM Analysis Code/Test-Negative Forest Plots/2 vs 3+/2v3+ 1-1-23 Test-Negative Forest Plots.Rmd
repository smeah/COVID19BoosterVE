...
```{r}
pdf("./Figures/TN_3+v2_Infection_Start_Quarter.pdf", width = 10, height = 6, onefile = FALSE)
...
```{r}
pdf("./Figures/TN_3+v2_Infection_QStrat.pdf", width = 10, height = 6, onefile = FALSE)
...
fig <- forestplot(
  ...
  fp_set_zebra_style("#EFEFEF")
legend(x = 0.4, y = 1.22, c("Adjusted", "Unadjusted"), title = "Adjustment", border = "#CCCCCC", box.lwd = 1.5, 
       col = c(pal[1], pal[2]), pch = c(15,15), bg = "white", horiz = TRUE, xpd = TRUE) 
...
```{r}
pdf("./Figures/TN_3+v2_Infection_Matched.pdf", width = 10, height = 6, onefile = FALSE)
...
fig <- forestplot(
  ...
  #title = "Test-Negative 3+ vs 2 Dose Infection VE with Different Matched Designs",
  xlab = "Vaccine Effectiveness (VE)",
  graph.pos = 4,
  boxsize = .2,
  clip = c(.25,.5),
