...
```{r}
Severe_NonMatched <- read.csv("3v4+ Severe Disease Non-Matched Forest Plot.csv") 
...
Severe_NonMatched <- Severe_NonMatched %>% 
  ...
  mutate(Design = seq(1, 18, by = 1)) 
Severe_Matched <- read.csv("3v4+ Severe Disease Matched Forest Plot.csv")
...
```{r}
pdf("./Figures/TN_4+v3_Severe_Start_Quarter.pdf", width = 10, height = 8, onefile = FALSE)
...
fig <- forestplot(
  ...
  #title = "Test-Negative 4+ vs 3 Dose Severe Disease VE With Different Non-Matched Designs",
  title = "",
...
```{r}
pdf("./Figures/TN_4+v3_Severe_Matched.pdf", width = 10, height = 8, onefile = FALSE)
...
fig <- forestplot(
  ...
  #title = "Test-Negative 4+ vs 3 Dose Severe Disease VE With Different Matched Designs (Q2 2022 Index Date)",
  title = "",
  xlab = "Vaccine Effectiveness (VE)",
  graph.pos = 3,
  boxsize = .2,
